<?php
  ...
  if (isset($_GET['acao']) && $_GET['acao']=='destaque') {
    echo "<script> window.alert('Imovel proposto para destaque'); </script>";
  }


  //verificar todos os imoveis deste gestor
  $file=fopen("../../data/$fileimoveis", "r");
  $id=0;
  while (!feof($file)) {
    $data=fgetcsv($file,0,";");
    if($data[1]==$_SESSION['admin']){
      $imoveis[]=$data[0];
    }
  }
  fclose($file);

  include'adicionar_imovel.php';
